...
export const FabRequestView: React.FC<{ 
...
}> = ({ language, onNotify, user, onRefreshNotifications }) => {
  const labs = {
  ...
  useEffect(() => {
    ...
    apiGet<FabRequestResponse[]>(`/api/fab/requests?requesterId=${encodeURIComponent(user.employeeId)}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user?.employeeId) {
      try {
        const saved = await apiPost<FabRequestResponse>('/api/fab/requests', {
        ...
        const newReq: RequestItem = {
          ...
          waferCount: saved.waferCount,
          status: saved.status,
          priority: saved.priority,
        };
        setRequests([newReq, ...requests]);
        onNotify(
          language === 'en' ? 'Request Created' : '憪??桀歇撱箇?',
